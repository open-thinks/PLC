    ...
    $.extend(altEditor.prototype, {
        ...
        _openEditModal: function () {
            ...
            for (var i = 0; i < dt.context[0].aoColumns.length; i++) {
                columnDefs.push({
                    ...
                    name: dt.context[0].aoColumns[i].sName,
                    editable: dt.context[0].aoColumns[i].editable
                })
            }
            ...
            for (var j in columnDefs) {
                data +=
                    "<div class='form-group'><div class='col-sm-3 col-md-3 col-lg-3 text-right' style='padding-top:7px;'><label for='" +
                    columnDefs[j].title + "'>" + columnDefs[j].title +
                    ":</label></div><div class='col-sm-9 col-md-9 col-lg-9'><input type='text'  id='" +
                    columnDefs[j].title + "' name='" + columnDefs[j].name + "' placeholder='" +
                    columnDefs[j].title +
                    "' style='overflow:hidden'  class='form-control  form-control-sm' value='" +
                    adata.data()[0][j] + "' " +
                    (columnDefs[j]["editable"] ? "" : "readonly=yes") +
        ...
        _editRowData: function () {
            ...
            var fnSuccess = function(){
                var message =
                    '<div class="alert alert-success" role="alert"><strong>Success!</strong> This record has been updated.</div>';
                $('#altEditor-modal .modal-body').append(message);
                dt.row({
                    selected: true
                }).data(data);
            };
            if (window.dte && window.dte.saveCallback) {
                window.dte.saveCallback.call(this, $('form[name="altEditor-form"]'),fnSuccess );
            } else {
							fnSuccess.call(this);
            }
        },
        ...
        _openDeleteModal: function () {
            ...
            for (var i in columnDefs) {
                ...
                data += "<div class='form-group'><label for='" + columnDefs[i].title + "'>" +
                    columnDefs[i]
                    .title + " : </label><input  type='hidden'  id='" + columnDefs[i].title +
                    "' name='" +
                    columnDefs[i].title + "' placeholder='" + columnDefs[i].title +
                    "' style='overflow:hidden'  class='form-control' value='" + adata.data()[0][i] +
                    "' >" +
                    adata.data()[0][i] + "</input></div>";
        ...
        _openAddModal: function () {
            ...
            for (var j in columnDefs) {
                data +=
                    ...
                    columnDefs[j].title + "'>" + columnDefs[j].title +
                    ":</label></div><div class='col-sm-9 col-md-9 col-lg-9'><input type='text'  id='" +
                    columnDefs[j].title + "' name='" + columnDefs[j].title + "' placeholder='" +
                    columnDefs[
                        j].title +
                    "' style='overflow:hidden'  class='form-control  form-control-sm' value=''></div><div style='clear:both;'></div></div>";
